          ...
          case Some(clo: CloV) => {
            if (args.length != clo.params.length) {
              ...
              val newSto = args.foldLeft(es) { (accSto, arg) =>
                ...
                val newAddr = generateNewAddress(updatedSto) // A function to generate a new address
                (updatedSto + (newAddr -> argVal), newAddr)
              }

              val (result, finalSto) = interpHelper(clo.body, newEnv, newSto)
              (result, finalSto)
            }
          }
          case Some(ConstructorV()) => {
            args.foreach{ arg =>
              val argv, args = interpHelper(arg, env, sto)
